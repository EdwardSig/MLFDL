...
def eval_mini_batch(batch_size: int, 
                    user_positive_interaction: list,
                    item_num: int, *tensors):
    """_summary_
    ...
    if len(tensors) == 1:
        tensor = tensors[0]
        for i in range(0, len(tensor), batch_size):
            batch_purchase_tensor = zeros(len(tensor[i:i + batch_size]), item_num)
            batch_tuple = (tensor[i:i + batch_size], )
            for pidx, b in enumerate(tensor[i:i + batch_size]):
                item_id_list = list(user_positive_interaction[b])
                batch_purchase_tensor[pidx, item_id_list] = 1
            yield (batch_tuple, batch_purchase_tensor)
    else:
        for i in range(0, len(tensors[0]), batch_size):
            batch_purchase_tensor = zeros(size=(len(tensors[0][i: i + batch_size]), item_num))
            batch_tuple = tuple(x[i:i + batch_size] for x in tensors)
            for pidx, b in enumerate(tensors[0][i:i + batch_size]):
                item_id_list = list(user_positive_interaction[b])  # 看用户给哪些物品打过分
                batch_purchase_tensor[pidx, item_id_list] = 1
            yield (batch_tuple, batch_purchase_tensor)
...
def draw_loss_pic_one_by_one(max_step: int,
...
def save_loss_pic(max_step: int,
                  filename: str,
                  use_random_color: bool = False,
                  **losses):
...
def save_loss_pic_one_by_one(max_step: int,
                             dir_path: str,
                             use_random_color: bool = False,
                             **losses):
    for key in losses.keys():
...
def show_me_all_the_fucking_result(raw_result: dict, metric_list: list,
                                   k_list: list, best_index: int) -> dict:
    result_dict: dict = dict()
    for metric in metric_list:
        for k in k_list:
            temp_array: np.array = np.array(
                merge_dict(raw_result[metric], _show_me_a_list_func)[k])
            dict_key: str = str(metric) + '@' + str(k)
            result_dict[dict_key] = temp_array[best_index]
    return result_dict


def show_me_all_the_fucking_explicit_result(raw_result: dict,
                                            metric_list: list,
                                            best_index: int) -> dict:
    result_dict: dict = dict()
    for metric in metric_list:
