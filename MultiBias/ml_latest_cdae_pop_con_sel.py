...
from models import BiasFactCDAEImplicit, BiasFactCDAEImplicitThreeBias
from train import ImplicitTrainManager, ImplicitTrainThreeBiasManager
from utils import draw_score_pic, merge_dict, _show_me_a_list_func, draw_loss_pic_one_by_one, query_user, query_str, mkdir, query_int, get_class_name_str, _mean_merge_dict_func, show_me_all_the_fucking_result
...
DATASET_PATH = 'ml-latest-cdae-pop-con-sel/'
...
def main(
    ...
    model: BiasFactCDAEImplicitThreeBias = BiasFactCDAEImplicitThreeBias(
        ...
        sel_env_num=model_config['sel_env_num'],
    ...
    pop_env_label = torch.LongTensor(data_loader.train_df['pop_label'].tolist())
    con_env_label = torch.LongTensor(data_loader.train_df['con_label'].tolist())
    sel_env_label = torch.LongTensor(data_loader.train_df['sel_label'].tolist())
    ...
    sel_env_classifier = np.zeros(shape=(sel_env_label.shape[0], len(sel_env_label.unique())))
    sel_env_classifier[[i for i in range(0, sel_env_label.shape[0])], sel_env_label.numpy()] = 1
    env_classifier = np.concatenate((pop_env_classifier, con_env_classifier, sel_env_classifier), axis=1)
    env_classifier = torch.FloatTensor(env_classifier)

    train_manager: ImplicitTrainThreeBiasManager = ImplicitTrainThreeBiasManager(
        model=model,
        evaluator=evaluator,
        training_data=train_tensor,
        pop_env_label=pop_env_label,
        con_env_label=con_env_label,
        sel_env_label=sel_env_label,
