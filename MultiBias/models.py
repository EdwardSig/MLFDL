...
class ExLinearLogSoftMaxEnvClassifier(EnvClassifier):

    def __init__(self, factor_dim, hidden_layers, env_num):
        super(ExLinearLogSoftMaxEnvClassifier, self).__init__()
        self.hidden_num = len(hidden_layers)
        hidden_list = []

        for i, hl in enumerate(hidden_layers):
            if i == 0:
                hidden_list.append(nn.Linear(factor_dim, hl))
                hidden_list.append(nn.ReLU())
            else:
                hidden_list.append(nn.Linear(hidden_layers[i-1], hl))
                hidden_list.append(nn.ReLU())
        if len(hidden_layers) > 0:
            self.linear_map: nn.Sequential = nn.Sequential(*hidden_list)
            self.bias_category: nn.Linear = nn.Linear(hidden_layers[-1], env_num)
            self.predict_score: nn.Linear = nn.Linear(hidden_layers[-1], 1)
        else:
            self.linear_map: nn.Linear = nn.Linear(factor_dim, env_num)
            self.predict_score: nn.Linear = nn.Linear(factor_dim, 1)
        # self.classifier_func = nn.LogSoftmax(dim=1)
        # self._init_weight()
        self.elements_num: float = float(factor_dim * env_num)
        self.bias_num: float = float(env_num)
    ...
    def forward(self, invariant_preferences):
        if self.hidden_num > 0:
            env_outputs = self.bias_category(self.linear_map(invariant_preferences))
            env_aware_score = self.predict_score(self.linear_map(invariant_preferences))
        else:
            env_outputs = self.linear_map(invariant_preferences)
            env_aware_score = self.predict_score(invariant_preferences)

        return env_aware_score, env_outputs
...
class InvPrefExplicit(GeneralDebiasExplicitRecommender):
    ...
    def __init__(self,
        ...
        self.envlassifier: EnvClassifier = LinearLogSoftMaxEnvClassifier(
            factor_num, env_num)
...
class BiasFactCDAEImplicit(GeneralDebiasImplicitRecommender):
    ...
    def __init__(self,
        ...
        self.item2hidden = nn.Sequential(nn.Linear(item_num, factor_num),
                                         nn.Dropout(drop_rate))
        self.user2hidden = nn.Embedding(user_num, factor_num)
        self.pop_env2hidden = nn.Embedding(pop_env_num, factor_num)  # 流行度环境向量, 用于环境识别
        self.con_env2hidden = nn.Embedding(con_env_num, factor_num)  # 一致性环境向量, 用于环境识别
        self.hidden2out = nn.Linear(factor_num, item_num)
    ...
    def forward(self, users_id, items_id, pop_envs_id, con_envs_id,
                cdae_train_user_ids, cdae_envs_id, cdae_envs_weight, purchase_vec, alpha=0):
        # cdae_envs_weight = cdae_envs_weight.reshape(cdae_envs_weight.shape[0], cdae_envs_weight.shape[1], 1)

        pop_env_embs = self.pop_env2hidden(cdae_envs_id[:, :, 0]) * cdae_envs_weight
        con_env_embs = self.con_env2hidden(cdae_envs_id[:, :, 1]) * cdae_envs_weight
        env_embs = pop_env_embs * con_env_embs
        cdae_env_emb = env_embs.sum(dim=1)
        ...
        return out, env_aware_score.reshape(-1), env_outputs
...
class BiasFactCDAEImplicitThreeBias(GeneralDebiasImplicitRecommender):
    """
    多标签+KDD作者+CDAE
    """
    ...
    def __init__(self,
                 ...
                 reg_only_embed: bool = False,
                 reg_env_embed: bool = True):
        super(BiasFactCDAEImplicitThreeBias, self).__init__(user_num=user_num,
                                                   item_num=item_num,
                                                   env_num=pop_env_num + con_env_num + sel_env_num)
        ...
        self.pop_env2hidden = nn.Embedding(pop_env_num, factor_num)  # 流行度环境向量, 用于环境识别
        self.con_env2hidden = nn.Embedding(con_env_num, factor_num)  # 一致性环境向量, 用于环境识别
        self.sel_env2hidden = nn.Embedding(sel_env_num, factor_num)  # 选择偏差环境向量, 用于环境识别
        self.hidden2out = nn.Linear(factor_num, item_num)
        ...
        # 环境分类器
        self.env_classifier: EnvClassifier = LinearLogSoftMaxEnvClassifier(
            factor_num, pop_env_num + con_env_num + sel_env_num)
        self.output_func = nn.Sigmoid()
    ...
    def _init_weight(self):
        ...
        nn.init.normal_(self.pop_env2hidden.weight, std=0.01)
        nn.init.normal_(self.con_env2hidden.weight, std=0.01)
        nn.init.normal_(self.sel_env2hidden.weight, std=0.01)
        nn.init.normal_(self.hidden2out.weight, std=0.01)
        nn.init.normal_(self.embed_user_env_aware.weight, std=0.01)
        nn.init.normal_(self.embed_item_env_aware.weight, std=0.01)

    def forward(self, users_id, items_id, pop_envs_id, con_envs_id, sel_envs_id,
                cdae_train_user_ids, cdae_envs_id, cdae_envs_weight, purchase_vec, alpha=0):
        # cdae_envs_weight = cdae_envs_weight.reshape(cdae_envs_weight.shape[0], cdae_envs_weight.shape[1], 1)

        pop_env_embs = self.pop_env2hidden(cdae_envs_id[:, :, 0]) * cdae_envs_weight
        con_env_embs = self.con_env2hidden(cdae_envs_id[:, :, 1]) * cdae_envs_weight
        sel_env_embs = self.sel_env2hidden(cdae_envs_id[:, :, 2]) * cdae_envs_weight
        env_embs = pop_env_embs * con_env_embs * sel_env_embs
        cdae_env_emb = env_embs.sum(dim=1)
        ...
        pop_envs_embed: torch.Tensor = self.pop_env2hidden(pop_envs_id)
        con_envs_embed: torch.Tensor = self.con_env2hidden(con_envs_id)
        sel_envs_embed: torch.Tensor = self.sel_env2hidden(sel_envs_id)
        envs_embed: torch.Tensor = pop_envs_embed * con_envs_embed * sel_envs_embed
    ...
    def get_sel_envs_reg(self, envs_id, norm: int):
        embed_gmf: torch.Tensor = self.sel_env2hidden(envs_id)
        if norm == 2:
            reg_loss: torch.Tensor = embed_gmf.norm(2).pow(2) / (
                    float(len(envs_id)) * float(self.factor_num))
        elif norm == 1:
    ...
    def get_L2_reg(self, users_id, items_id, pop_envs_id, con_envs_id, sel_envs_id):
        if not self.reg_only_embed:
            result = self.env_classifier.get_L2_reg()
            result = result + (self.get_users_reg(users_id, 2) +
                               self.get_items_reg(items_id, 2))
        ...
        if self.reg_env_embed:
            result = result + self.get_pop_envs_reg(pop_envs_id, 2) + self.get_con_envs_reg(con_envs_id, 2) + self.get_sel_envs_reg(sel_envs_id, 2)
        # print('L2', result)
        return result

    def get_L1_reg(self, users_id, items_id, pop_envs_id, con_envs_id, sel_envs_id):
        if not self.reg_only_embed:
            result = self.env_classifier.get_L1_reg()
            result = result + (self.get_users_reg(users_id, 1) +
                               self.get_items_reg(items_id, 1))
            ...
            result = self.get_users_reg(users_id, 1) + self.get_items_reg(
                items_id, 1)
        if self.reg_env_embed:
            result = result + self.get_pop_envs_reg(pop_envs_id, 1) + self.get_con_envs_reg(con_envs_id, 1) + self.get_sel_envs_reg(sel_envs_id, 1)
        # print('L2', result)
        return result
...
class BiasFactMFExplicitThreeBias(GeneralDebiasExplicitRecommender):
    ...
    def __init__(self,
                 ...
                 reg_only_embed: bool = False,
                 reg_env_embed: bool = True):
        super(BiasFactMFExplicitThreeBias, self).__init__(user_num=user_num,
                                                   item_num=item_num,
                                                   env_num=pop_env_num+con_env_num+sel_env_num)
        self.factor_num: int = factor_num
        ...
        self.pop_env2hidden = nn.Embedding(pop_env_num, factor_num)
        self.con_env2hidden = nn.Embedding(con_env_num, factor_num)
        self.sel_env2hidden = nn.Embedding(sel_env_num, factor_num)
    ...
    def forward(self, users_id, items_id, pop_envs_id, con_envs_id, sel_envs_id, alpha=0):
