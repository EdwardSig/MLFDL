...
class BiasFactCDAEImplicitAmazon(GeneralDebiasImplicitRecommender):
...
class CDAEImplicit(GeneralDebiasImplicitRecommender):
    ...
    def __init__(self,
                 ...
                 reg_only_embed: bool = False,
                 reg_env_embed: bool = True):
        super(CDAEImplicit, self).__init__(user_num=user_num,
                                           item_num=item_num,
                                           env_num=env_num)
    ...
    def forward(self, cdae_train_user_ids, purchase_vec, alpha=0):

        ###############################################
        # CDAE
        ###############################################
        hidden = self.output_func(
            self.coe1 * self.user2hidden(cdae_train_user_ids).squeeze(dim=1) +
            self.coe1 * self.item2hidden(purchase_vec))
        out = self.hidden2out(hidden)
        out = self.output_func(out)
    ...
    def get_users_reg(self, users_id, norm: int):
        ...
        print("您在运行get_users_reg方法，该方法不合法请停止！！！")
        sys.exit()
    ...
    def get_items_reg(self, items_id, norm: int):
        ...
        print("您在运行get_items_reg方法，该方法不合法请停止！！！")
        sys.exit()
    ...
    def get_envs_reg(self, envs_id, norm: int):
        ...
        print("您在运行get_envs_reg方法，该方法不合法请停止！！！")
        sys.exit()
    ...
    def get_L2_reg(self, users_id, items_id, envs_id):
        ...
        print("您在运行get_L2_reg方法，该方法不合法请停止！！！")
        sys.exit()
    ...
    def get_L1_reg(self, users_id, items_id, envs_id):
        ...
        print("您在运行get_L1_reg方法，该方法不合法请停止！！！")
        sys.exit()
    ...
    def predict(self, users_id, purchase_vec):
        ...
        ###############################################
        hidden = self.output_func(
            self.coe2 * self.user2hidden(users_id).squeeze(dim=1) +
            self.coe2 * self.item2hidden(purchase_vec))
        out = self.hidden2out(hidden)
        out = self.output_func(out)
    ...
    def cluster_predict(self, users_id, items_id, envs_id) -> torch.Tensor:
        ...
        print("您在运行cluster_predicit方法，该方法不合法请停止！！！")
        sys.exit()
