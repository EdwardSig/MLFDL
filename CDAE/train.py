...
from evaluate import ImplicitTestManager, ExplicitTestManager, PDAImplicitTestManager
...
class ImplicitTrainManagerNone:
    def __init__(
            self, model: GeneralDebiasImplicitRecommender, evaluator: ImplicitTestManager,
            device: torch.device,
        ...
        for i in range(self.batch_num):
            ...
            self.batch_cdae_list.append((users_unique_tensor, cdae_envs_id, cdae_envs_weight, purchase_tensor))
            # # 内存不够，用文件的方式处理
            # tmp_tuple = (users_unique_tensor, cdae_envs_id, cdae_envs_weight, purchase_tensor)
            # with open(cdea_pickle_path+'cdea_pickle_file'+str(i)+'.pkl', 'wb') as f:
            #     pickle.dump(tmp_tuple, f)
        print('zhjeli   gggggggg')
        for env in range(self.envs_num):
            envs_tensor: torch.Tensor = torch.LongTensor(np.full(training_data.shape[0], env, dtype=int))
            envs_tensor = envs_tensor.to(self.device)
            self.const_env_tensor_list.append(envs_tensor)
    ...
    def train_a_batch(
        ...
        # # 内存不够，用文件的方式处理
        # with open(self.cdae_pickle_path+'cdea_pickle_file'+str(batch_index)+'.pkl', 'rb') as f:
        #     tmp_tuple = pickle.load(f)
        ...
        # # 获取不变兴趣特征预测的分数, 可变特征预测的分数, 环境分类器预测的分类结构
        # out = self.model(
        #     users_unique_tensor, purchase_tensor, alpha
        # )
        ...
        cdae_loss: torch.Tensor = 5 * torch.sum(recommend_loss(out, self.batch_cdae_list[batch_index][3]))
...
class ImplicitTrainManagerAmazon:
    def __init__(
            self, model: GeneralDebiasImplicitRecommender, evaluator: ImplicitTestManager,
            device: torch.device,
            training_data: torch.Tensor, env_label: torch.Tensor, batch_size: int,
    ...
    def train_a_batch(
        ...
        cdae_loss: torch.Tensor = torch.sum(recommend_loss(out1, self.batch_cdae_list[batch_index][3]))
        env_aware_loss: torch.Tensor = recommend_loss(env_aware_score, batch_scores_tensor)
        envs_loss: torch.Tensor = env_loss(env_outputs, batch_envs_tensor)
        ...
        loss: torch.Tensor = self.invariant_coe * cdae_loss + env_aware_loss * self.env_aware_coe \
                             + envs_loss * self.env_coe + L2_reg * self.L2_coe + L1_reg * self.L1_coe
...
class PDAImplicitTrainManager:
    def __init__(
            self, model: GeneralDebiasImplicitRecommender, evaluator: PDAImplicitTestManager, device: torch.device,
            training_data: torch.Tensor, batch_size: int, epochs: int,
            evaluate_interval: int, lr: float, test_begin_epoch: int = 0):
        self.model: GeneralDebiasImplicitRecommender = model
        self.evaluator: PDAImplicitTestManager = evaluator
        self.envs_num: int = self.model.env_num
        self.device: torch.device = device
        self.users_tensor: torch.Tensor = training_data[:, 0]
        self.items_tensor: torch.Tensor = training_data[:, 1]
        ...
        for i in range(self.batch_num):
            ...
            for pidx, uid_tensor in enumerate(users_unique_tensor):
                ...
                item_ids = batch_items_tensor[user_idxs]
                purchase_tensor[pidx, item_ids] = batch_scores_tensor[user_idxs]
            self.batch_cdae_list.append((users_unique_tensor, purchase_tensor))
    ...
    def train_a_batch(
        ...
        cdae_loss: torch.Tensor = 5 * torch.sum(recommend_loss(out1, self.batch_cdae_list[batch_index][1]))
        ...
        loss: torch.Tensor = cdae_loss + reg_loss
