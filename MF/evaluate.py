...
class ExplicitTestManagerDraw:
    ...
    def evaluate(self) -> dict:
        ...
        for (batch_index, (batch_users_tensor, batch_users_list, batch_users_ground_truth)) in \
            ...
            result_pred_items_list.extend(batch_result)
        # print(result_dicts_list)

        if self.eval_epoch == 10:
            with open('dataset/preprocessing/deviation_mix/ml-100k-pred_items.pkl', 'wb') as f:
                pickle.dump(result_pred_items_list, f)
            print('文件保存成功! 程序退出。')
            sys.exit()
        ...
        self.eval_epoch += 10
        reformat_result_dict = {}

        return reformat_result_dict


class PDAExplicitTestManager:
